var icon = document.querySelector("#iconChrome"), shape = document.querySelector("#shapeChorme"), 
mini = document.getElementById("shapeMini"), click_me = document.getElementById("clickMe");
			...
			case "top": {
				...
				*/
				...
				points = [
					...
					[rect.right, rect.top],
					[rect.right, rect.bottom - height/offset_rate], // 距离右上角3/4
...
var animate = function(from, to, direction, callback) {
	...
	for (var index=0; index<target.length-1; index++) {
		...
		arr_pos_123.forEach(function(arr_pos, index_trigger) {
			if (index_match == -1 && arr_pos.indexOf(index) != -1) {
				// index_match = 0 1 2 2 2 1 0 0
				index_match = index_trigger;
			}
		});
	
		/*
		 * index	index_match									rate
		 *   0		     0				(target左上y - trigger左上y) / (target左上x - trigger左上x)
		 *   1		     1				(target中上y - trigger中上y) / (target中上x - trigger中上x)
		 *   2		     2				(target右上y - trigger右上y) / (target右上x - trigger右上x)
		 *   3		     2				(target右中y - trigger右上y) / (target右中x - trigger右上x)
		 *   4		     2				(target右下y - trigger右上y) / (target右下x - trigger右上x)
		 *   5		     1				(target中下y - trigger中上y) / (target中下x - trigger中上x)
		 *   6		     0				(target左下y - trigger左上y) / (target左下x - trigger左上x)
		 *   7		     0				(target左中y - trigger左上y) / (target左中x - trigger左上x)
		 */
		// 斜率
	...
	}, 1000, mina[from.length == 3? "easein": "easeout"], callback);	
};
...
var display = {
	...
	show: function() {
		...
		// 还原	
		// get.point(icon, rand_position)-->上面三个点
		// get.points(shape, rand_position)-->屏幕中间图片的9个点
		// direction	vertical
